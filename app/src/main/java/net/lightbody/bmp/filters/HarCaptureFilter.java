...
public class HarCaptureFilter extends HttpsAwareFiltersAdapter {
    ...
    private static final String TAG = HarCaptureFilter.class.getSimpleName();
    ...
    /**
     * The requestCaptureFilter captures all request content, including headers, trailing headers, and content. The
     * HarCaptureFilter
     * delegates to it when the clientToProxyRequest() callback is invoked. If this request does not need content
     * capture, the
    ...
    /**
     * Like requestCaptureFilter above, HarCaptureFilter delegates to responseCaptureFilter to capture response
     * contents. If content capture
    ...
    /**
     * Populated by proxyToServerResolutionStarted when DNS resolution starts. If any previous filters already
     * resolved the address, their resolution time
    ...
    /**
     * The address of the client making the request. Captured in the constructor and used when calculating and
     * capturing ssl handshake and connect
     * timing information for SSL connections.
     */
    private final InetSocketAddress clientAddress;

    /**
     * Request body size is determined by the actual size of the data the client sends. The filter does not use the
     * Content-Length header to determine request size.
     */
    ...
    /**
     * The "real" original request, as captured by the
     * {@link #clientToProxyRequest(io.netty.handler.codec.http.HttpObject)} method.
     */
    private volatile HttpRequest capturedOriginalRequest;

    /**
     * True if this filter instance processed a
     * {@link #proxyToServerResolutionSucceeded(String, java.net.InetSocketAddress)} call, indicating
    ...
    /**
     * Create a new instance of the HarCaptureFilter that will capture request and response information. If no har is
     * specified in the
     ...
     * @param har             a reference to the ProxyServer's current HAR file at the time this request is received
     *                        (can be null if HAR capture is not required)
     * @param currentPageRef  the ProxyServer's currentPageRef at the time this request is received from the client
     * @param dataToCapture   the data types to capture for this request. null or empty set indicates only basic
     *                        information will be
     *                        captured (see {@link net.lightbody.bmp.proxy.CaptureType} for information on data
     *                        collected for each CaptureType)
     */
    public HarCaptureFilter(HttpRequest originalRequest, ChannelHandlerContext ctx, Har har, String currentPageRef,
        ...
        // we may need to capture both the request and the response, so set up the request/response filters and
        // delegate to them when
        ...
        if (this.dataToCapture.contains(CaptureType.REQUEST_CONTENT) || this.dataToCapture
                .contains(CaptureType.REQUEST_BINARY_CONTENT)) {
            requestCaptureFilter = new ClientRequestCaptureFilter(originalRequest);
        } else {
            requestCaptureFilter = null;
        }

        if (this.dataToCapture.contains(CaptureType.RESPONSE_CONTENT) || this.dataToCapture
                .contains(CaptureType.RESPONSE_BINARY_CONTENT)) {
    ...
    public HttpResponse clientToProxyRequest(HttpObject httpObject) {
        ...
        // configured, we still need to capture basic information (timings, possibly client headers, etc.), just not
        // content.
        ...
        if (httpObject instanceof HttpRequest) {
            // link the object up now, before we make the request, so that if we get cut off (ie: favicon.ico request
            ...
            // create a "no response received" HarResponse, in case the connection is interrupted, terminated, or the
            ...
            // connect times are recorded in the first request between the client and remote server after the HTTP
            // CONNECT.
        ...
        if (httpObject instanceof LastHttpContent) {
            ...
            if (dataToCapture.contains(CaptureType.REQUEST_CONTENT)) {
                captureRequestContent(requestCaptureFilter.getHttpRequest(),
                        requestCaptureFilter.getFullRequestContents());
            }
    ...
    public HttpObject serverToProxyResponse(HttpObject httpObject) {
        ...
        if (httpObject instanceof LastHttpContent) {
            if (dataToCapture.contains(CaptureType.RESPONSE_CONTENT)) {
                captureResponseContent(responseCaptureFilter.getHttpResponse(),
                        responseCaptureFilter.getFullResponseContents());
            }
    ...
    protected void captureQueryParameters(HttpRequest httpRequest) {
        // capture query parameters. it is safe to assume the query string is UTF-8, since it "should" be in US-ASCII
        // (a subset of UTF-8),
        ...
        try {
            ...
            // QueryStringDecoder will throw an IllegalArgumentException if it cannot interpret a query string.
            // rather than cause the entire request to
    ...
    protected void captureUserAgent(HttpRequest httpRequest) {
        ...
        if (har.getLog().getBrowser() == null) {
            ...
            if (userAgentHeader != null && userAgentHeader.length() > 0) {
                try {
                    //                    ReadableUserAgent uai = BrowserMobProxyUtil.getUserAgentStringParser()
                    // .parse(userAgentHeader);
                    //                    String browser = uai.getName();
                    //                    String version = uai.getVersionNumber().toVersionString();
                    har.getLog().setBrowser(new HarNameVersion(
                            "Mozilla/5.0 (Linux; U; Android 4.3; zh-cn; R8007 Build/JLS36C) AppleWebKit/534.30 "
    ...
    protected void captureRequestHeaderSize(HttpRequest httpRequest) {
        String requestLine =
                httpRequest.getMethod().toString() + ' ' + httpRequest.getUri() + ' ' + httpRequest.getProtocolVersion()
                        .toString();
    ...
    protected void captureRequestContent(HttpRequest httpRequest, byte[] fullMessage) {
        ...
        if (contentType == null) {
            log.warn("No content type specified in request to {}. Content will be treated as {}", httpRequest.getUri(),
                    BrowserMobHttpUtil.UNKNOWN_CONTENT_TYPE);
        ...
        try {
            ...
            log.warn(
                    "Found unsupported character set in Content-Type header '{}' in HTTP request to {}. Content will "
                            + "not be captured in HAR.",
                    contentType, httpRequest.getUri(), e);
            return;
        }

        if (charset == null) {
            // no charset specified, so use the default -- but log a message since this might not encode the data
            // correctly
    ...
    protected void captureResponseContent(HttpResponse httpResponse, byte[] fullMessage) {
        ...
        if (responseCaptureFilter.isResponseCompressed() && !responseCaptureFilter.isDecompressionSuccessful()) {
            log.warn("Unable to decompress content with encoding: {}. Contents will be encoded as base64 binary data.",
        ...
        try {
            ...
            log.warn(
                    "Found unsupported character set in Content-Type header '{}' in HTTP response from {}. Content "
                            + "will not be captured in HAR.",
        ...
        if (!forceBinary && BrowserMobHttpUtil.hasTextualContent(contentType)) {
            ...
            harEntry.getResponse().getContent()
                    .setText(new String(DatatypeConverter.parseBase64Binary(new String(fullMessage))));
    ...
    protected void captureResponseCookies(HttpResponse httpResponse) {
        ...
        for (String setCookieHeader : setCookieHeaders) {
            ...
            if (cookie.maxAge() > 0) {
                ...
                expires.setTimeInMillis(
                        expires.getTimeInMillis() + TimeUnit.MILLISECONDS.convert(cookie.maxAge(), TimeUnit.SECONDS));
    ...
    protected void captureConnectTiming() {
        HttpConnectTiming httpConnectTiming =
                HttpConnectHarCaptureFilter.consumeConnectTimingForConnection(clientAddress);
    ...
    protected void populateAddressFromCache(HttpRequest httpRequest) {
        ...
        if (serverHost != null && !serverHost.isEmpty()) {
            ...
            if (resolvedAddress != null) {
                ...
                // the resolvedAddress may be null if the ResolvedHostnameCacheFilter has expired the entry (which is
                // unlikely),
                // or in the far more common case that the proxy is using a chained proxy to connect to connect to the
                // remote host. since the chained proxy handles IP address resolution, the IP address in the HAR must
                // be blank.
    ...
    public void proxyToServerConnectionSucceeded(ChannelHandlerContext serverCtx) {
        ...
        // make sure the previous timestamp was captured, to avoid setting an absurd value in the har (see
        // serverToProxyResponseReceiving())
        if (connectionStartedNanos > 0L) {
            harEntry.getTimings()
                    .setConnect(connectionSucceededTimeNanos - connectionStartedNanos, TimeUnit.NANOSECONDS);
        } else {
    ...
    public void proxyToServerRequestSending() {
        ...
        // if the hostname was not resolved (and thus the IP address populated in the har) during this request,
        // populate the IP address from the cache
        if (!addressResolved) {
    ...
    public void serverToProxyResponseReceiving() {
        ...
        // started to receive response, so populate the 'wait' time. if we started receiving a response from the
        // server before we finished
        // sending (for example, the server replied with a 404 while we were uploading a large file), there was no
        // wait time, so
    ...
    public void serverToProxyResponseReceived() {
        ...
        // like the wait time, the receive time requires that the serverToProxyResponseReceiving() method be called
        // before this method is invoked.
        // typically that should happen, but it has been reported (https://github
        // .com/lightbody/browsermob-proxy/issues/288) that it
        // sometimes does not. therefore, to be safe, make sure responseReceiveStartedNanos is populated before
        // setting the receive time.
