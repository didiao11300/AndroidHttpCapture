...
/**
 * A filter adapter for {@link ResponseFilter} implementations. Executes the filter when the
 * {@link HttpFilters#serverToProxyResponse(HttpObject)}
 * method is invoked.
 */
public class ResponseFilterAdapter extends HttpsAwareFiltersAdapter implements ModifiedRequestAwareFilter {
    private final ResponseFilter responseFilter;
    private final static String TAG = ResponseFilterAdapter.class.getSimpleName();
    ...
    public ResponseFilterAdapter(HttpRequest originalRequest, ChannelHandlerContext ctx,
    ...
    public HttpObject serverToProxyResponse(HttpObject httpObject) {
        ...
        if (httpObject instanceof HttpResponse) {
            ...
            if (httpObject instanceof FullHttpMessage) {
                ...
                // the HTTP object is not a FullHttpMessage, which means that message contents will not be available
                // on this response and cannot be modified.
                contents = null;
            }

            HttpMessageInfo messageInfo =
                    new HttpMessageInfo(originalRequest, ctx, isHttps(), getFullUrl(modifiedHttpRequest),
                            getOriginalUrl());
    ...
    /**
     * A {@link HttpFiltersSourceAdapter} for {@link ResponseFilterAdapter}s. By default, this FilterSource enables
    ...
    public static class FilterSource extends HttpFiltersSourceAdapter {
         ...
         * {@link #DEFAULT_MAXIMUM_RESPONSE_BUFFER_SIZE} as
        ...
        /**
         * Creates a new filter source that will invoke the specified filter and uses the
         * maximumResponseBufferSizeInBytes as the maximum
         * buffer size. Set maximumResponseBufferSizeInBytes to 0 to disable aggregation. <b>If message aggregation
         * is disabled,
         * the {@link HttpMessageContents} will not be available for modification.</b> (<b>Note:</b> HTTP message
         * aggregation will
